...
describe("HomePage with shallow test", () => {
  afterEach(cleanup);
  ...
  test("should check Footer Text", () => {
    const { getByText } = render(<HomePage />);
    const linkElement = getByText(/Developed /i);
    expect(linkElement).toBeInTheDocument();
  });

  test("should have Called Filter Component", () => {
    expect(container.find("Filter").length).toEqual(1);
  });

  test("should have proper props for Filter Component", () => {
    expect(container.find("Filter").props()).toEqual({
      onChange: expect.any(Function),
    });
  });
});

describe("HomePage with mount", () => {
  afterEach(cleanup);
  const setRespArr = jest.fn();
  const container = mount(<HomePage respArr={[]} />);
  const spyStateChange = jest.spyOn(React, "useState");
  spyStateChange.mockImplementation((respArr) => [respArr, setRespArr]);

  test("should check initial state for respArr to be []", () => {
    expect(container.find("Card").length).toEqual(0);
  });

  test("should set state for respArr and check length", () => {
    setRespArr(mockRespArr);
    setTimeout(() => {
      expect(container.find("Card").length).toBe(3);
    }, 10);
  });
});
